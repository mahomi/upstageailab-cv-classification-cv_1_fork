...
class ImageDataset(Dataset):
    def __init__(self, csv_data, path, transform=None, cache_images=True, cache_augmented=True, augmentation_multiplier=1):
        ...
        if self.augmentation_multiplier > 1:
            ...
            # 증강된 데이터 추가
            for i, (name, target) in enumerate(original_df):
                for aug_idx in range(1, self.augmentation_multiplier):
                    augmented_data.append((name, target, aug_idx))  # aug_idx는 증강 인덱스
            
            self.df = np.array(augmented_data)
            log.info(f"📈 Data augmented: {len(original_df)} → {len(self.df)} samples (x{self.augmentation_multiplier})")
    ...
    def __getitem__(self, idx):
        ...
        if self.cache_augmented and self.augmented_cache is not None and cache_key in self.augmented_cache:
            ...
            # 증강 적용 (aug_idx가 0이면 원본, 1 이상이면 증강)
            if self.transform and aug_idx > 0:
                ...
                # 원본 이미지도 transform 적용 (resize, normalize 등)
                img = self.transform(image=img)['image']
...
trn_transform = A.Compose([
    ...
    ], p=0.3),    A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.2, p=0.5),
    A.OneOf([
...
for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df['target'])):
    ...
    # 데이터셋 생성
    trn_dataset = ImageDataset(train_fold, f"{data_path}/train/", transform=trn_transform, augmentation_multiplier=AUGMENTATION_MULTIPLIER)
    val_dataset = ImageDataset(val_fold, f"{data_path}/train/", transform=val_transform, augmentation_multiplier=AUGMENTATION_MULTIPLIER)
...
pred_df.to_csv(f"{output_path}/pred_advanced_kfold_tta2_efnv2rwm_v3.csv", index=False)

log.info(f"\n✅ Ensemble prediction completed and saved to {output_path}/pred_advanced_kfold_tta2_efnv2rwm_v3.csv")
